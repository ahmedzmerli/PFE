...
public class JimDashboardService {
    ...
    @PersistenceContext(unitName = "jimDashboard")
    ...
    public List<JimDashboard> searchDashboard(
        ...
        predicates.add(cb.equal(root.get("numClient"), msisdn));
        predicates.add(cb.equal(root.get("hotline"), hotline));

        if (startDate != null) {
            predicates.add(cb.greaterThanOrEqualTo(root.get("dateHeurs"), startDate));
        }
        if (endDate != null) {
            predicates.add(cb.lessThanOrEqualTo(root.get("dateHeurs"), endDate));
        }
